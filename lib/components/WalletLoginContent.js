...
export default class WalletLoginContent extends DomNode {
    ...
    async handleLogin(walletId) {
        try {
            ...
            const nonce = await SupabaseConnector.callEdgeFunction("api/wallet/new-nonce", { walletAddress });
            ...
            const token = await SupabaseConnector.callEdgeFunction("api/wallet/login", {
                walletAddress,
                signedMessage,
            });
