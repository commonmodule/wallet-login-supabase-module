...
export default class WalletLoginContent extends DomNode {
    ...
    async handleLogin(walletId) {
        try {
            ...
            const nonce = await WalletLoginConfig.supabaseConnector.callEdgeFunction("generate-wallet-login-nonce", { walletAddress });
            ...
            const token = await WalletLoginConfig.supabaseConnector.callEdgeFunction("wallet-login", {
                walletAddress,
                signedMessage,
            });
